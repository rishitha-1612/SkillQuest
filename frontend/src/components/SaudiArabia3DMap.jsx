...
import saudiArabiaData from '../data/saudi-arabia.json';

const SAUDI_SKILL_REGIONS = {
  prompt_engineering: 'Riyadh Region',
  machine_learning: 'Eastern Region',
  api_integration: 'Makkah Region',
  data_visualization: "'Asir Region",
  python_programming: 'Al Madinah Region',
};

const ACTIVE_REGION_COLORS = {
  'Riyadh Region': '#b6e3c6',
  'Eastern Region': '#d7f0f7',
  'Makkah Region': '#cfe9b6',
  "'Asir Region": '#9fd9c0',
  'Al Madinah Region': '#fde9a9',
};
...
const ACTIVE_DEPTH = 0.5;
const INACTIVE_DEPTH = 0.24;
...
export default function SaudiArabia3DMap({ roleDetails, stateById, selectedStateId, onStateSelect }) {
  ...
  const regionSkillMap = useMemo(() => {
    const pairs = (roleDetails?.state_requirements || [])
      .map((req) => {
        const regionName = SAUDI_SKILL_REGIONS[req.state_id];
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.Fog(0xeaf6f3, 60, 160);
    ...
    const seaGeo = new THREE.CircleGeometry(80, 64);
    ...
    const features = saudiArabiaData.features;
    let minLng = Infinity;
    let maxLng = -Infinity;
    let minLat = Infinity;
    let maxLat = -Infinity;
    ...
    features.forEach((feature) => {
      ...
      polygons.forEach((polygon) => {
        ...
        for (let holeIndex = 1; holeIndex < polygon.length; holeIndex += 1) {
          if (polygon[holeIndex] && polygon[holeIndex].length >= 3) {
      ...
      if (mappedSkill) {
        ...
        label.className = 'saudi-subject-label';
    ...
    const tick = () => {
      ...
      regions.forEach((region, index) => {
        ...
        if (region.isActive) {
          ...
          const breathe = Math.sin(elapsed * 0.8 + region.idlePhase) * 0.04;
          ...
          region.group.traverse((object) => {
            ...
            if (topMaterial.emissive) {
              topMaterial.emissive.copy(black).lerp(hoverEmissive, region.hoverT * 0.45);
            }
          });
        } else {
          region.group.position.z = Math.sin(elapsed * 0.55 + region.idlePhase) * 0.02;
        }
      });

      scene.rotation.x = Math.sin(elapsed * 0.25) * 0.012;
      scene.rotation.y = Math.cos(elapsed * 0.2) * 0.018;
  ...
  return (
    <div className="saudi-3d-map-shell">
      <div className="saudi-3d-map-stage">
        <div ref={mountRef} className="saudi-3d-map-canvas" />
        <div ref={labelsRef} className="saudi-3d-map-labels" />
        <div className="saudi-3d-map-badge">
          <span>Saudi Arabia 3D Skill Map</span>
