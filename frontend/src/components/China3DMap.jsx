...
const CHINA_SKILL_REGIONS = {
  networking_fundamentals: 'Guangdong',
  cybersecurity_fundamentals: 'Shandong',
  cloud_platforms: 'Zhejiang',
  devsecops_security: 'Sichuan',
  system_design: 'Xinjiang',
};
...
export default function China3DMap({ roleDetails, stateById, selectedStateId, onStateSelect }) {
  ...
  const regionSkillMap = useMemo(() => {
    const pairs = (roleDetails?.state_requirements || [])
      .map((req) => {
        const regionName = CHINA_SKILL_REGIONS[req.state_id];
  ...
  useEffect(() => {
    ...
    features.forEach((feature) => {
      ...
      if (mappedSkill) {
        ...
        label.className = 'china-subject-label';
  ...
  return (
    <div className="china-3d-map-shell">
      <div className="china-3d-map-stage">
        <div ref={mountRef} className="china-3d-map-canvas" />
        <div ref={labelsRef} className="china-3d-map-labels" />
        <div className="china-3d-map-badge">
          <span>China 3D Skill Map</span>
